...
typedef struct __attribute__((packed)) {
    ...
    uint8_t     serialProtocol:4,
                failsafeMode:2,
                unused:2;
    v9_rx_config_pwm_t pwmChannels[16] __attribute__((aligned(4)));
...
// V10 added flight-control PID fields, angle mode and PWM mixerMode.
typedef union {
    struct {
        uint32_t failsafe:10,
                 inputChannel:4,
                 inverted:1,
                 mode:4,
                 narrow:1,
                 failsafeMode:2,
                 signalPolarityInverted:1,
                 mixerMode:1,
                 unused:8;
    } val;
    uint32_t raw;
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t     serialProtocol:4,
                failsafeMode:2,
                unused:2;
    v10_rx_config_pwm_t pwmChannels[16] __attribute__((aligned(4)));
    uint8_t     teamraceChannel:4,
                teamracePosition:3,
                teamracePitMode:1;
    uint8_t     targetSysId;
    uint8_t     sourceSysId;
    int16_t     flightControlRatePid[12];
    int16_t     flightControlAnglePid[12];
